...
public class ResultActivity extends Activity {
    ...
    ArrayList<MyTImage>images1;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_result_layout);
        //images = (ArrayList<TImage>) getIntent().getSerializableExtra("images");
        images1 = (ArrayList<MyTImage>) getIntent().getSerializableExtra("images");
        showImg();
    }
    ///storage/emulated/0/temp/1547363681805.jpg
...
//    }
    ...
    private void showImg() {
        LinearLayout linearLayout = (LinearLayout) findViewById(R.id.llImages);
        for (int i = 0, j = images1.size(); i < j - 1; i += 2) {
            View view = LayoutInflater.from(this).inflate(R.layout.image_show, null);
            ImageView imageView1 = (ImageView) view.findViewById(R.id.imgShow1);
            ImageView imageView2 = (ImageView) view.findViewById(R.id.imgShow2);
            Glide.with(this).load(new File(images1.get(i).getLocalPath())).into(imageView1);
            Glide.with(this).load(new File(images1.get(i + 1).getLocalPath())).into(imageView2);
            linearLayout.addView(view);
        }
        if (images1.size() % 2 == 1) {
            View view = LayoutInflater.from(this).inflate(R.layout.image_show, null);
            ImageView imageView1 = (ImageView) view.findViewById(R.id.imgShow1);
            Glide.with(this).load(new File(images1.get(images1.size() - 1).getLocalPath())).into(imageView1);
            linearLayout.addView(view);
        }
